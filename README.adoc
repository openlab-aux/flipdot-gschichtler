...
frontend and API simultaneously for testing.
...
`nixos/warteraum.nix` provides a NixOS module which
defines `services.warteraum` for running the warteraum service
with reasonably reduced privileges. A minimal
`configuration.nix` utilizing it could look like this:
...
{
  imports = [
    flipdot-gschichtler.nixosModule
  ];

  services.warteraum = {
    enable = true;
    package = flipdot-gschichtler.warteraum;
    tokensFile = "/var/secrets/flipdot-gschichtler/tokens";
    saltFile = "/var/secrets/flipdot-gschichtler/salt";
  };

  services.nginx.virtualHosts."flipdot.pseudo.invalid" = {
    root = flipdot-gschichtler.bahnhofshalle;
    extraConfig = ''
      location /api {
